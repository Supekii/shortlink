...
public class IndexManageDaoImpl {
    ...
    public Map<String, Object> getTenantInfoById(String tenant_id) {
        try {
            String sql = "select * from s_tenant where tenant_id = ?";
            Map<String, Object> result = jdbcTemplate.queryForMap(sql, tenant_id);
            return result;
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    ...
    public int updatePasswordForFind(String tenantAccount, String tenantPassword) {
        ...
        String sql = "update s_tenant set tenant_password = ? where tenant_account = ?";
        return jdbcTemplate.update(sql, tenantPassword, tenantAccount);
    }
    ...
    public int updatePassword(String tenant_id, String tenant_password, String new_tenant_password) {
        String sql = "update s_tenant set tenant_password = ? where tenant_id = ? and tenant_password = ?";
        return jdbcTemplate.update(sql, new_tenant_password, tenant_id, tenant_password);
    }
